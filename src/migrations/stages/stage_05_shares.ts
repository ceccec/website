...
/**
 * **Shares** — time-limited links to posts/pages/case-studies (`src/collections/Shares.ts`).
 * Polymorphic `resource` uses `shares_rels` (same pattern as `redirects` / `redirects_rels`).
 *
 * Primary keys and FKs use **`text`** to match **`idType: 'uuidv7'`** (`PAYLOAD_SQL_ID_TYPE` / `payloadDB.ts`).
 */
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE INDEX IF NOT EXISTS \`shares_rels_case_studies_id_idx\` ON \`shares_rels\` (\`case_studies_id\`);`)
  ...
  try {
    ...
    // Column may already exist if schema was pushed / migration re-run.
  }
  await db.run(
    sql`CREATE INDEX IF NOT EXISTS \`payload_locked_documents_rels_shares_id_idx\` ON \`payload_locked_documents_rels\` (\`shares_id\`);`,
  )
}

export async function down(_args: MigrateDownArgs): Promise<void> {
  // SQLite: leave tables in place.
}
