...
export const TENANT_RELATION_FIELD = 'tenant' as const

const TENANTS_COLLECTION = 'tenants' as CollectionSlug

function extractRelationshipId(value: unknown): string | number | undefined {
...
function extractManyRelationshipIds(value: unknown): (string | number)[] {
...
export function tenantIdsForContentVisibility(
  ...
  tenantDoc: Record<string, unknown> | null | undefined,
): (string | number)[] {
...
export async function resolveContentTenantIdsForHost(
  ...
  return tenantIdsForContentVisibility(tenant.id as string | number, tenant)
}
...
export async function listTenantIdsForHostDomainChain(
  ...
  type Scored = { id: string | number; idx: number }
  const scored: Scored[] = []
...
export async function listAncestorTenantIds(
  ...
  const seen = new Set<string>()
  let current: string | number | undefined = tenantId
...
async function findTenantChildrenBatch(
  ...
  for (let i = 0; i < parentIds.length; i += PARENT_IN_CHUNK) {
    ...
    for (const doc of res.docs) {
      if (doc === null || typeof doc !== 'object' || !('id' in doc) || doc.id == null) continue
      collected.push(doc.id as string | number)
    }
  }
  return collected
}
...
export async function unionTenantScopeIds(
  payload: Payload,
  input: { host?: string; rootTenantId?: string | number },
): Promise<(string | number)[]> {
  if (!multiTenantEnabled()) return []
...
export function whereTenantIn(ids: (string | number)[], fieldName = TENANT_RELATION_FIELD): Where {
  return {
    [fieldName]: {
      in: ids,
    },
